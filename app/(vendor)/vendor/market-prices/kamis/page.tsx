 ...
 * @description 도매점 KAMIS 시세 참고 페이지
 *
 * 이 페이지는 도매점이 자신이 등록한 상품의 KAMIS 공영시장 시세를 비교하고 가격 경쟁력을 분석하는 페이지입니다.
 */
...
export default function VendorKamisMarketPricesPage() {
  ...
  const fetchMarketPriceForProduct = async (product: ProductWithMapping) => {
    ...
    try {
      ...
      console.group(`📊 ${product.original_name} KAMIS 시세 조회 시작`);
      console.log("표준 상품명:", standardProduct.standard_name);

      const response = await fetch(
        `/api/market-prices/kamis?productName=${encodeURIComponent(standardProduct.standard_name)}`,
      );
      const result = await response.json();
  ...
  const fetchAllMarketPrices = async () => {
    ...
    console.group("📊 모든 상품 KAMIS 시세 일괄 조회 시작");
    toast.info(
  ...
  const handleSearchMarketPrice = async () => {
    ...
    try {
      ...
      console.group("📊 KAMIS 시세 검색 시작");
      console.log("상품명:", searchQuery.trim());

      const apiUrl = `/api/market-prices/kamis?productName=${encodeURIComponent(searchQuery.trim())}`;
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h1 className="text-3xl font-bold">시세 참고 (KAMIS)</h1>
        </div>
        <p className="text-gray-600">
          내 상품의 KAMIS 공영시장 시세를 확인하고 가격 경쟁력을 분석하세요.
        </p>
      </div>
                ...
                <CardContent className="space-y-4">
                  ...
                  {product.marketPrices && product.marketPrices.length > 0 ? (
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <h3 className="text-lg font-semibold">KAMIS 시세</h3>
                    ...
                    <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                      <p className="text-sm text-gray-600">
                        시세 조회 버튼을 클릭하여 KAMIS 시세를 확인하세요.
                      </p>
                    </div>
                  )}
                </CardContent>
